...
const CURRENT_VENDOR_ID = "vendor-test-001"; // 目前先統一使用的 vendorId，之後若確定正式值再改
...
export default function ManagersPageClient() { // 匯出 managers 主頁元件
  ...
  const fetchMembers = useCallback(async () => { // 定義讀取協助人員名單的方法
    ...
    try { // 進入錯誤處理區塊
      const response = await fetch(`${MANAGERS_LIST_API}?vendorId=${encodeURIComponent(CURRENT_VENDOR_ID)}`, { // 呼叫正式 managers list API
        method: "GET", // 使用 GET
        cache: "no-store", // 不快取
      ...
      setPageInfo(`目前廠商：${CURRENT_VENDOR_ID}`); // 顯示目前使用的 vendorId
  ...
  const startLineBind = useCallback(async () => { // 定義啟動 LINE 綁定的方法
    ...
    try { // 進入錯誤處理
      ...
      const url = `${LINE_START_API}?flow=${encodeURIComponent(flow)}&preferSameTab=${shouldUseSameTab ? "1" : "0"}&vendorId=${encodeURIComponent(CURRENT_VENDOR_ID)}`; // 組出 start API 網址，並帶入 vendorId

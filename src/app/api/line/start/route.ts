...
const VENDOR_ID = process.env.BOOTSTRAP_VENDOR_ID || "vendor-yogo-001"; // 讀取目前廠商代號，若未設定就使用預設值
  ...
  try { // 開始錯誤捕捉
    ...
    if (missingKeys.length > 0) { // 若有任一必要環境變數缺漏
      return NextResponse.json( // 回傳錯誤 JSON
        {
          ok: false, // 標記此次請求失敗
          message: `LINE 環境變數未設定完整：${missingKeys.join("、")}`, // 顯示缺漏的環境變數名稱
        },
        { status: 500 } // 設定 HTTP 狀態碼為 500
      ); // 回傳結束
    } // 環境變數檢查結束

    const body = await request.json().catch(() => ({})); // 讀取前端傳來的 JSON，若解析失敗則使用空物件
    ...
    const isMobile = Boolean(body.isMobile); // 取得是否為手機或同頁跳轉模式
    const effectiveLoginMode = isMobile ? "normal" : loginMode; // 若是手機或同頁跳轉模式，強制改用 normal，避免進 QR 流程
    const state = createLineState(VENDOR_ID, flow); // 產生 LINE callback 要用的 state
    const authUrl = new URL("https://access.line.me/oauth2/v2.1/authorize"); // 建立 LINE OAuth 授權網址物件

    authUrl.searchParams.set("response_type", "code"); // 設定 OAuth response_type 為 code
    ...
    if (effectiveLoginMode === "qr") { // 若目前模式是桌機 QR Code 登入
    ...
    return NextResponse.json({ // 回傳成功 JSON 與 LINE 登入網址
      ok: true, // 標記成功
      loginUrl: authUrl.toString(), // 回傳 LINE 登入網址
      mode: effectiveLoginMode, // 回傳實際使用模式，方便除錯
      flow, // 回傳目前流程，方便除錯
    }); // 回傳結束
